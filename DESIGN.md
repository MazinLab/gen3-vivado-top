...
We suggest starting from [mkidgen](https://github.com/MazinLab/MKIDGen3) if you are looking at this project for the first time. This repository focuses on the gateware. This document and project are in heavy flux and resource calculations are only partially done. Some of the linked projects do not yet exist or are untested (especially in the timekeeping/triggering area of the design) and the names and links are suggestive placeholders. In some cases there are alternative/backup approaches that are not described.

*See the readme for information on how to create a vivado project of the design and how to get started.*
...
4. 256 MHz capture clock. Generated by from an MMCM in a clock wizard from the 128MHz clock.
5. 333 MHz DDR4 clock. Used to read/write from the PL DDR4. Generated by the MIG.

## Resets
Tied off for most of the design. The RFDC and the dac_replay blocks require them for proper functionality.

### Stream Capture
AXI4S broadcasters are used in various hierarchies to route the data streams to the capture subsystem, key cores are  [pair_iq (adc capture)](https://github.com/MazinLab/iq-capture), filter_iq, or filter_phase. A stream switch is used to pick between adc, iq, or phase streams and then they are fed to Dan Gisselquist's lovely axis2mm verilog core which generates the AXI transactions. 
