...
class PeripheralTestCase(unittest.TestCase):
    ...
    def test_frame_counter(self):
        ...
        async def testbench(ctx):
            ...
            await ctx.tick()
            self.assertEqual(ctx.get(dut.cuber_peri.cuber.generate_cubes), 0)
            await write_cpf(5000)
            await ctx.tick().repeat(5)
            self.assertEqual(ctx.get(dut.cuber_peri.cuber.cycles_per_frame), 5000)
            await ctx.tick()
            await generate_sample_pixel_LUT()
            await generate_sample_wavelength_LUT()
            await ctx.tick()
            await write_generate(1)
            await ctx.tick().repeat(5)
            self.assertEqual(await _csr_access(self, ctx, dut.cuber_peri.bus, addr=dut.cuber_peri.bus.memory_map.find_resource(dut.cuber_peri._framecounter).start, r_stb=1, w_stb=0, w_data=0), 0)
        ...
        with sim.write_vcd("test_frame_counter.vcd"):
            sim.run()